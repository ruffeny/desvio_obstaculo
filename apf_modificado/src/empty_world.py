...
def modified_potential_field(goal,obstacles,obstacles_radius,obstacles_velocities,current_position,epsilon,Nd,Ns,Ne,tau,Ros,Dsafe,phou0,vos_velocity):
	...
	Nog = pg/dg  
	print("current_position = ",current_position,"pg = ",pg," dg = ",dg," Nog = ",Nog)
	#initiate Fatt
	...
	for i,obstacle in enumerate(obstacles):
		...
		d, Dm, CR, Pot, theta_m_radian, Vto, tetha, Not, AngleDiff, Notperp, Rts = iniciation(obstacles[i], current_position, vos_velocity, obstacles_velocities[i], obstacles_radius[i], Dsafe, phou0)
...
obstacles = [np.array([-2, 2]), np.array([-6, 2]), np.array([-2, 6]), np.array([-6, 6])]#amb 31
#obstacles = [np.array([7.8, 2.2]), np.array([6.8, 4.9]), np.array([7, 0]), np.array([7.5, 7]), np.array([6, 8]), np.array([4, 8])] #amb 7

#define the obstacle radius
obstacles_radius = np.array([0.5, 0.5, 0.5, 0.5]) #amb 31
#obstacles_radius = np.array([0.5, 0.5, 0.5, 0.5,0.5,0.5]) #amb 7

#define the obstcale velocities
obstacles_velocities = [np.array([0, 0]), np.array([0, 0]), np.array([0, 0]), np.array([0, 0])] #amb 31
#obstacles_velocities = [np.array([0, 0]), np.array([0, 0]), np.array([-0.3, 0.3]), np.array([-0.25, 0.25]), np.array([0, -0.8*0.11]), np.array([1.8*0.0875,1.8* (-0.05)])]#amb 7

amb = 1
obst.change_position()
...
while not rospy.is_shutdown():
	...
	total_force = modified_potential_field(goal_pos,obstacles,obstacles_radius,obstacles_velocities,robot_pos,epsilon,Nd,Ns,Ne,tau,Ros,Dsafe,phou0,vos_velocity)
	...
	#print("speed = ",speed)
	print("v = ",speed.linear.x," w = ",speed.angular.z*(180 / math.pi))
