...
def setParam():
	...
	Rts = 0.3; #mn
	vmax = 0.4; #m/s
	maxturn = vmax/Ros ; #5 graus #math.radians(30) #
	wmax = maxturn
	Dm = Ros + Dsafe + Rts;
	CR = Dm + phou0;
...
while not rospy.is_shutdown():
	...
	obst.save_robot_position(obst_positions,x_box, y_box)
	...
	if distance_to_goal < 0.5:
		...
		if k == 1:
			obst.plot_robot_positions(robot_positions,obst_positions,Dm)
			k = K + 1
		break
	if i>5:
		pub.publish(speed)
